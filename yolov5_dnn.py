...
class yolov5():
    def __init__(self, onnx_path, confThreshold=0.25, nmsThreshold=0.45):
        self.classes = ['simple_room','person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat',
                        'traffic light',
    ...
    def detect(self, srcimg):
        ...
        for i in pred[0]:
            ...
            # print(width-left,height-top)
            conf = i[4]
            classId = i[5]
            # 计算距离
            dis = self.GetDistens(width-left, height-top)
            dis = '%.2f' % dis
            ...
            label = '%s:%s' % (self.classes[int(classId)], dis+'m')
            # Display the label at the top of the bounding box
            labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 3)
...
def create_file(content):
    ...
    # with open(r"data2.txt", "w") as file:
    file = open(r"test_img/detect_img_name.txt", "a+", encoding='utf-8')
    file.write(content + '\n')
    file.close()
def mult_test(onnx_path, img_dir, save_root_path, video=False):
    ...
    if video:
        ...
        # print(os.walk(img_dir))
        All_img_list = ScanFile(img_dir)
        ''' 1 首先 卡一下时间'''
        today_img=[] # save today img
        for img in All_img_list:
            img_time = os.path.getmtime(img)
            # 时间戳 => 结构化时间 => 时间字符串
            img_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(img_time))
            # print(img_time)
            date = time.time()
            date = time.strftime('%Y-%m-%d', time.localtime(date))
            if img_time>date:
                if img.endswith(".png") or img.endswith(".jpg") or img.endswith(".jpeg") or img.endswith(
                        ".PNG") or img.endswith(".JPG") or img.endswith(".JPEG"):
                    today_img.append(img)

        ''' 2 找到最近图像'''
        Latest_images = max(today_img, key=os.path.getctime)
        # print(max(today_img, key=os.path.getctime))

        ''' 判断最新图像是否已经进行检测'''
        txt = open(r"test_img/detect_img_name.txt",encoding='utf-8')
        detect_img_list = txt.readlines()
        # print(detect_img_list)
        if Latest_images+ '\n' in detect_img_list:
            return 0
        # 开始检测

        srcimg = cv2.imread(Latest_images)
        srcimg,flag = model.detect(srcimg)
        print("  finish:   ", Latest_images)
        create_file(Latest_images)
        # 若果检测到目标则保存
        if flag:
            save_path = os.path.join(save_root_path, Latest_images.split('/')[-1])
